...
const emailSender = async (
  ...
  return new Promise(async (resolve, reject) => { // 추가된 부분
    ...
    try {
      ...
      const saveMailOptionsToFirestore = async (
        ...
        try {
          ...
          console.error('Firestore에 데이터를 저장하는 중 오류가 발생했습니다:', error);
        }
      };
      ...
      schedule.scheduleJob(reservationDate, async () => {
        try {
          ...
          console.error('예약된 날짜에 메일 보내는 과정에서 에러가 발생했습니다(1). :', error);
